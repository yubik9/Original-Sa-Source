...
char *aszStatus[] = { "¥þ", "¬r", "³Â", "¯v", "¥Û", "¾K", "¶Ã", "µê", "¼@", "»Ù", "Àq" 
...
char *aszStatusFull[] = { "¥þ§Ö", "¬r", "³Â·ô", "ºÎ¯v", "¥Û¤Æ", "°s¾K", "²V¶Ã", "µê®z", "¼@¬r", "Å]»Ù", "¨IÀq" 
#ifdef _PET_SKILL_SARS			// WON ADD ¬r·Ù½¯©µ
					  ,"¬r·Ù"
#endif
#ifdef _PROFESSION_SKILL			// WON ADD ¤Hª«Â¾·~§Þ¯à
					  ,"·w¯t","ÄñÂ¶","¤ÑÃ¹","¦BÃz","¿ò§Ñ","¦B½b","¶Ý¦åÅÛ","¤@°w¨£¦å","¬D¼·","¤õªþÅé",
					  "¦BªþÅé","¹pªþÅé","±Mª`¾Ô°«","¤õ§Ü","¦B§Ü","¹p§Ü","¤õªþ","¦Bªþ","¹pªþ"
#endif
};
...
#endif
#ifdef _PROFESSION_ADDSKILL
						,CHAR_WORKERSIST_F_I_T, CHAR_WORKWATER//¤ôªþÅé
						,CHAR_WORKANNEX//ªþ¨­
						,CHAR_WORKFEAR//®£Äß
						,CHAR_WORKICECRACK2
						,CHAR_WORKICECRACK3
...
char *aszParamChange[] = { "µL", "§ð", "¨¾", "¦­", "¾y", "®·" };
char *aszParamChangeFull[] = { "µL", "§ðÀ»¤O", "¨¾¿m¤O", "³t«×", "¾y¤O", "®·Àò²v" };
...
float BATTLE_adjustRidePet3A( int charaindex, int petindex, int workindex, int action )
{
	...
	switch( workindex )	{
	case CHAR_WORKATTACKPOWER : // §ðÀ»¤O
		if( throwweapon ) // §ëÂY
            ret = CHAR_getWorkInt( charaindex, CHAR_WORKATTACKPOWER );//¶¡±µ§ðÀ»:¤Hª«
		else // ªñ¾Ô
            ret = CHAR_getWorkInt( petindex, CHAR_WORKATTACKPOWER );//ª½±µ§ðÀ»:Ãdª«
		break;
...
)
{
	...
	if( BATTLE_CanMoveCheck( defindex ) == FALSE ){
#ifdef _PROFESSION_ADDSKILL
		if( CHAR_getWorkInt( defindex, CHAR_DOOMTIME ) <= 0 )//¶°®ð®É¥i°{Á×(¨Ã¥B¨S¦³³B©ó¤ÑÃ¹¦aºô©M¬ÞÀ»ªºª¬ºA)
			return FALSE;
		else{
...
#ifdef _PROFESSION_ADDSKILL
	if( CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_CHAOS ){ //²V¶Ã§ðÀ»®É©R¤¤­°§C
		per += (per*0.4);	
	}
#endif
...
#ifdef _EQUIT_NEGLECTGUARD //©¿µø¥Ø¼Ð¨¾¿m¤O%
...
#ifdef _ADD_DEAMGEDEFC //ÃB¥~¶Ë®` & ªþ¥[§ðÀ»
	{
...
// Terry add fix ¨Ï¥Î±Ë¤w¬°¤H§Þ¯à®É,­Y§ðÀ»¨Ï¥Î¦¹§Þ¯àªº¤Hclient·|·í
...
#endif
#ifdef _PROFESSION_ADDSKILL
	|| CHAR_getWorkInt( GuardianIndex, CHAR_DOOMTIME ) > 0 //¥@¬É¥½¤é¶°®ð
#endif
	){
...
static int BATTLE_AttackSeq( int attackindex, int defindex, int *pDamage, int *pGuardian,	int opt )
{
	...
	if( opt == BATTLE_COM_S_DAMAGETOHP2 ){ //·t¤ë¨g¯T (·|¤ß¤@À»¤W¤É30%)  && CHAR_getInt( attackindex, CHAR_HP) <= CHAR_getWorkInt( attackindex, CHAR_MAXMP)*0.5
		//print("\n1·|¤ß¤@À»:%d",perCri);                 
		//print("\n1§ð:%d", CHAR_getWorkInt( attackindex, CHAR_WORKATTACKPOWER));
		//print("\n1±Ó:%d", CHAR_getWorkInt( attackindex, CHAR_WORKQUICK));
		perCri = perCri + (perCri*0.3); //·|¤ß¤@À»¤W¤É30%  
		CHAR_setWorkInt( attackindex, CHAR_WORKATTACKPOWER, CHAR_getWorkInt( attackindex,CHAR_WORKFIXSTR)+CHAR_getWorkInt( attackindex,CHAR_WORKFIXSTR)*0.2);//§ð¤W¤É20%
		...
		//print("\n2·|¤ß¤@À»:%d",perCri);
		//print("\n2§ð:%d", CHAR_getWorkInt( attackindex, CHAR_WORKATTACKPOWER));
...
#endif
#ifdef _PROFESSION_ADDSKILL
					|| CHAR_getWorkInt( attackindex, CHAR_WORKANNEX) > 0 //ªþ¨­
#endif
				){					//§ðÀ»¤è²§±`²V¶Ã
...
int BATTLE_DamageSub( int attackindex, int defindex, int *pDamage, int *pPetDamage, int *pRefrect )
{
	...
	if( *pRefrect == BATTLE_MD_ACUPUNCTURE ){ //°w¨ë¥~¥Ö
		...
		//¥ý¦©³Q§ðÀ»¤èªº¦å,¦]¬°¤§«áªº°Ê§@³£¬O¹ï§ðÀ»¤è°µªº
		hp = CHAR_getInt( defindex, CHAR_HP );
		if( attackpet == -1 ) {
			hp -= damage;
			if(hp<0)hp=0;
...
#ifdef _PETSKILL_FIREKILL //¤õ½uÂy±þ±M¥Î
	...
	for( i=0; i<MAXCAPTRUEFREE; i++ ){
		...
		for( j = 0; j < CHAR_MAXITEMHAVE; j ++ ){
			...
			if( ITEM_getInt( itemindex, ITEM_ID ) == NeedEnemy[ ti].ItemId[i] ){
				...
				//break;//Change fix §ì¨ì¤@°¦Ãd¥u§R°£¤@­Ó¹D¨ã(³Ì«áÁÙ¬O¨M©w¥þ§R)
			}
		}
	}
	return TRUE;
...
)
{
	...
	{
		char szBuffer[256]="";
		//snprintf( szBuffer, sizeof(szBuffer), "(%s)«O¦í¤F¦Û¨­¦w¥þ¡C",
		//	CHAR_getUseName( attackindex )
		//);
		//BATTLE_BroadCast( battleindex, szBuffer,
		//	(attackNo >= 10)? CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
...
int BATTLE_S_GBreak( int battleindex, int attackNo, int defNo )
{
	...
	case BATTLE_RET_CRITICAL:
		//snprintf( szBuffer, sizeof(szBuffer),
		//"(%s)¨ü¨ì(%s)ªºµ´§ÞCRITICAL(%d)·l¶Ë(%d:%d:%d%%)",
		//	CHAR_getUseName( attackindex ),
		//	CHAR_getUseName( defindex ),
		//	damage,
		//	CHAR_getWorkInt( attackindex, CHAR_WORKATTACKPOWER ),
...
int BATTLE_S_Roar( int battleindex, int attackNo, int defNo, int marray )
{
	...
	index2 = BATTLE_No2Index( battleindex, defNo);//³QÀ»¤¤¥Ø¼Ð¤§index
...
int BATTLE_Combo( int battleindex, int *pAttackList, int defNo )
{
	...
	//BATTLE_BroadCast( battleindex, "µo°Ê¥²±þ§Þ¡I¡I",
...
int BATTLE_S_FallGround( int battleindex, int attackNo, int defNo, int skill_type)
{
	...
	BOOL iRet = FALSE;
	attackindex = BATTLE_No2Index( battleindex, attackNo );	//§ð¤èindex
...
#ifdef _SKILL_TOOTH
	case BATTLE_COM_S_TOOTHCRUSHE:	//ø¥¾¦³N tooth
		flg |= BCF_TOOTH;
...
int battle_profession_attack_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	switch( skill_type){		
		...
		case BATTLE_COM_S_CHAOS:		// ²V¶Ã§ðÀ»
			{
				int atk = 0;
				// ´î30%§ðÀ»¤O
		...
		case BATTLE_COM_S_CHAIN_ATK:	// ³sÀô§ðÀ»
			{
				int rand_num = RAND(1, 100);
				int hit; 
		...
		case BATTLE_COM_S_CHAIN_ATK_2:	// Âù­«§ðÀ»
			{
				int atkpower = 0;
				...
				//if( reflec_num > 0 )	CHAR_setWorkInt( defindex, CHAR_WORKDAMAGEREFLEC, max(reflec_num - 1, 0)  );//ÅýÃè¦³®Ä,³o¦æ­n®³±¼
		...
		case BATTLE_COM_S_DEAD_ATTACK:	// Ãx¦º§ðÀ»
			{
				...
				if( old_hp <= 10 ){
					CHAR_talkToCli( charaindex, -1, "HP¤£¨¬¡AµLªk¨Ï¥Î§Þ¯à", CHAR_COLORYELLOW);
					return iRet;
				}
				...
				CHAR_talkToCli( charaindex, -1, "©R¤¤²v¤Wª@", CHAR_COLORYELLOW);
		...
		case BATTLE_COM_S_ATTACK_WEAK:		// ®zÂI§ðÀ»
			{
		...
		case BATTLE_COM_S_PLUNDER:			// «ÍÅé±°¹Ü
			{	
...
//			skill_type=-1;
	...
	//¨ú±o§ðÀ»«áª¬ºA
	iWork = BATTLE_AttackSeq( attackindex, defindex, &damage, &Guardian, skill_type );
	if( skill_type == BATTLE_COM_S_ENRAGE_PET ){//Åý¿E«ãÃdª«§Þ¯à¤£­n¯uªº¥´¦ºÃdª«
	...
	ultimate = BATTLE_DamageSub( attackindex, defindex, &damage, &petdamage, &react);
#else
	if( skill_type == BATTLE_COM_S_CAVALRY)		// ®yÃM§ðÀ»
	...
	switch( skill_type){
		...
		case BATTLE_COM_S_DEAD_ATTACK:	// Ãx¦º§ðÀ»
		case BATTLE_COM_S_ATTACK_WEAK:	// ®zÂI§ðÀ»
		case BATTLE_COM_S_CAVALRY:		// ®yÃM§ðÀ»
			{
				snprintf( szCommand, sizeof( szCommand ), "B+|a%X|r%X|f%X|d%X|p%X|e%X|s%X|h%X|FF|",
...
int battle_profession_attack_magic_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	// §ðÀ»½d³ò 0³æÅé 1³æ±Æ 2¤@¤è¥þÅé
	if( toindex < 20 ){					// ³æÅé
		attIdx = 0;
	}else if( toindex>=23 && toindex<=26 ){ // ³æ±Æ
		attIdx = 1;
	}else if( toindex==20 || toindex==21 ){	// ¤@¤è¥þÅé
		attIdx = 2;
	}

	// Robin fix ¼W¥[¾Ô³õÄÝ©ÊÀu¶Õ
...
int battle_profession_assist_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	switch(skill_type){
		...
		case BATTLE_COM_S_ENRAGE:			// ¿E¤Æ§ðÀ»
			{
				int str=0, tgh=0;
				skill_level = PROFESSION_CHANGE_SKILL_LEVEL_A( skill_level );
...
int battle_profession_status_chang_fun(int battleindex, int attackNo, int defNo, int charaindex)
{
	...
	switch(skill_type){	
		// ¤£²¾°Ê«¬
		case BATTLE_COM_S_ENTWINE:			// ¾ð®ÚÄñÂ¶
...
#endif
		case BATTLE_COM_S_FIRE_ENCLOSE:		// ¤õªþÅé
		case BATTLE_COM_S_ICE_ENCLOSE:		// ¦BªþÅé
		case BATTLE_COM_S_THUNDER_ENCLOSE:	// ¹pªþÅé
			{
				...
				for( i = 0; ToList[i] != -1; i ++ ){
					...
					if( (PROFESSION_BATTLE_StatusAttackCheck( charaindex, toindex, status, Success ) == 0 )
						...
						if( status == BATTLE_ST_DRAGNET ){//¤ÑÃ¹¦aºô
							CHAR_setWorkInt( toindex, CHAR_DOOMTIME, 0 );//¥@¬É¥½¤é¶°®ð
						...
						if( skill_type == BATTLE_COM_S_FIRE_ENCLOSE ){			// ¤õªþÅé
						...
						}else if(skill_type == BATTLE_COM_S_ICE_ENCLOSE ){		// ¦BªþÅé
						...
						}else if(skill_type == BATTLE_COM_S_THUNDER_ENCLOSE ){	// ¹pªþÅé
		...
		// ²¾°Ê«¬
		case BATTLE_COM_S_SHIELD_ATTACK:	// ¬ÞÀ»
			{
				int itmid=-1;
				...
				if( ( itmid < 0 ) || ITEM_WSHIELD != ITEM_getInt( itmid , ITEM_TYPE ) ){
					CHAR_talkToCli( charaindex, -1, "¥¼¸Ë³Æ¬ÞµP¡AµLªk¨Ï¥Î¦¹§Þ¯à", CHAR_COLORYELLOW);
					iRet = FALSE;	
					break;
				} 				
		...
		case BATTLE_COM_S_BOUNDARY:         // ¥|ÄÝ©Êµ²¬É
            {
				...
				if( ((pszP = strstr( pszOption, "¯}µ²¬É" ) ) == NULL ) ){//¥ý§â©Ò¦³µ²¬É²M°£
...
typedef struct _tsAttackObject{
	int index;					// §ðÀ»ª«¥ó½s¸¹
	int target;					// ¥Ø¼Ð½s¸¹
	int actionNumber;		// §ðÀ»ª«¥ó¹Ï¸¹
}AttackObject;
// BATTLE_BattleModel ¥Îªº§ðÀ»¨ç¦¡
	...
	int iType)
{
	...
	// iType & 0x00000004 ¬° true ªí¥Ü¬°ª«²z§ðÀ»
	if(iType & 0x00000004){
		// ­Y©¾¤ü¨S¦º¤`,³]©w©¾¤ü
		if(BATTLE_TargetCheck(battleindex,iGuardian)){
			// ­Y¦³©¾¤ü,§â³Q§ðÀ»ªÌ³]©w¬°¨Ï¥Î©¾¤ü§Þ¯àªº¨¤¦â,§_«h³Q§ðÀ»ªÌ¬°­ì¥Ø¼Ð
	...
	// ¦b³o¤§«áªº iDefindex ¤~¬O¯u¥¿·|¨ü¶Ëªº¥Ø¼Ð
	...
	if(CHAR_getInt(iDefindex,CHAR_HP) <= 0){
		...
		if(iDamage > 0 && BATTLE_StatusAttackCheck(charaindex,iDefindex,iEffect,iEffectHit,30,1.0,&iTemp)){
			...
			// ¦^¶Çclient¨º¤@­Ó¥Ø¼Ð­n°õ¦æª¬ºA§ïÅÜ
			if(iGuardian >= 0) BATTLE_BadStatusString(iGuardian,iEffect);	// iGuardian ¤j©óµ¥©ó 0 ªí¥Ü¦³¥Ø¼Ð¨Ï¥Î¤F©¾¤ü,¸Ó¥Ø¼Ðª¬ºA§ïÅÜ
			else BATTLE_BadStatusString(pAAttackObject->target,iEffect);	// §_«h­ì¥Ø¼Ðª¬ºA§ïÅÜ
		}
	}
...
void BATTLE_BattleModel(int battleindex,int attackNo,int myside)
{
	...
	iType = CHAR_GETWORKINT_LOW(charaindex,CHAR_WORKBATTLECOM2);				// ¨ú±o§ðÀ»Ãþ«¬
	iObjectNum = CHAR_GETWORKINT_HIGH(charaindex,CHAR_WORKBATTLECOM2);	// ¨ú±o§ðÀ»ª«¥ó¼Æ¶q
	...
	// §PÂ_¥Ø¼Ð¼Æ¶q©M§ðÀ»ª«¥ó¼Æ¶q
	// ·í§ðÀ»ª«¥ó¼Æ¶q¤j©óµ¥©ó¦b³õ¼Ä¤è¼Æ¶q¡A«h§ðÀ»©Ò¦³¥Ø¼Ð¡A¦³ªº¥Ø¼Ð¥i¯à¨ü¨ì¤G¦¸¥H¤Wªº§ðÀ»
	if(iObjectNum >= i){
		i0 = i;		// °O¿ý³õ¤W¥Ø¼Ð°¦¼Æ

		for(i=0;i<iObjectNum;i++){
			// ­Y iToList[i] ¬° -1 ªí¥Ü¤w¥´§¹©Ò¦³ªº¥Ø¼Ð
		...
		// ÁÙ¦³§ðÀ»ª«¥ó¨S¦³¥Ø¼Ð¥i¥H§ðÀ»,¶i¦æÀH¾÷¥Ø¼Ð§ðÀ»
		for(;i<iObjectNum;i++){
			// ³]©w§ðÀ»¥Ø¼Ð¤º®e
			if(AAttackObject[i].index != -1){
	...
	// ·í§ðÀ»ª«¥ó¼Æ¶q¤p©ó¦b³õ¥Ø¼Ð¼Æ¶q¡A«h¥H§ðÀ»Ãþ«¬¨M©w§ðÀ»¤è¦¡
	else{
		i0 = i;		// °O¿ý³õ¤W¥Ø¼Ð°¦¼Æ
		// «¬ºA 2 :¨C­Ó§ðÀ»ª«¥ó¥´§¹¸Ó§ðÀ»¥Ø¼Ð«á«K¤£¦A§ðÀ»¨ä¥¦¥Ø¼Ð,¨C­Ó¥Ø¼Ð¥u·|¨ü¤@¦¸§ðÀ»
		...
		// «¬ºA 1 :¨C­Ó¥Ø¼Ð³£­n¥´¨ì,·|¦³¤@­Ó§ðÀ»ª«¥ó¥´¤G­Ó¥Ø¼Ð¥H¤Wªº±¡§Î,©Ò¥H­Y¬°«¬ºA 1 ªº¸Ü,°£¤F«¬ºA 2 ªº§ðÀ»³B²z­n°õ¥~,­n¦A§@¥H¤U³B²z
		if(iType & 0x00000001){
			// ³B²zÁÙ¨S¦³³Q¥´ªº¥Ø¼Ð
			// i0 - iObjectNum ¬°³Ñ¤U¨S¥´ªº¥Ø¼Ð¼Æ¶q
			for(i1=0;i1<i0 - iObjectNum;i1++){	// ­pºâ³Ñ´X­Ó¥Ø¼Ð¨S¥´
				for(i2=0;i2<iObjectNum;i2++){			// §ðÀ»ª«¥ó­p¼Æ
					// ­Y iToList[i] ¬° -1 ªí¥Ü¤w¥´§¹©Ò¦³ªº¥Ø¼Ð
