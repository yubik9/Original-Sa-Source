...
//*********************************************************
{
	...
	// 爵お正旦弁毛綜嶽允月
	battleindex = BATTLE_CreateBattle(  );
	if( battleindex < 0 )return BATTLE_ERR_NOTASK;// 正旦弁�站咫リ迨吨咫瓣縞言憿�

	// 仇切日及諷採反皿伊奶乩□諷採匹丐月午爛聒允月﹝
	BattleArray[battleindex].Side[0].type = BATTLE_S_TYPE_PLAYER;
	// 輊仇丹礎及諷採手皿伊奶乩□諷採匹丐月午爛聒允月﹝
	BattleArray[battleindex].Side[1].type = BATTLE_S_TYPE_PLAYER;
	// 伉□母□毛忡繡
	BattleArray[battleindex].leaderindex = charaindex;
	// 爵お及正奶皿
...
//#define RS_LIST_MAX	4	// き凜卞窒諦爵お瑛絆霜月井
...
int BATTLE_GetDuelPoint(
	int battleindex,	// 爵お奶件犯永弁旦
	int side, 			// 扔奶玉﹋ㄟ℅ㄠ﹌
	int num 			// 憤坌反т昫及窒�筇雂峊郊膆擗�
)
{
	char szBuffer[1024]="";
	int charaindex;
...
//*********************************************************
//
// 爵お醱﹜輔爵仄凶化皿伊奶乩□及心互戲五化中月橇謫井毛民尼永弁
//
...
//************************************************************
{
	...
	case BATTLE_COM_ITEM:	// 失奶氾丞毛銀丹樺寧
		work = CHAR_getWorkInt( charaindex, CHAR_WORKQUICK )+20;
...
//*************************************************************
{
	...
	for( i = 0; i < entrynum; i ++ ){
		...
		if( start != -1 ){		// 寧蟲僎猾�紜冕�
			if(	com != BATTLE_COM_ATTACK 	// 僎猾戊穴件玉匹卅中
			||	enemy != oldenemy 			// 謊勻化月襯互き元匹卅中
			||	side != oldside 			// 扔奶玉互啜丹
			||	armtype == 1 				// 髑仆煙匹丐月
			||	move == 0 					// が仃卅中
			){
...
//*************************************************************
//
//  戊件申生□扑亦件互呿癲卞匹五月井民尼永弁
//﹛慼及諦互戊件示匹五月橇謫元扎卅井勻凶日 FALSE
//
static BOOL ComboCheck2(
	BATTLE_CHARLIST *pEntryList,	// 巨件玄伉□伉旦玄
	int nownum,		// 蜇箕及賜��
	int entrynum	// 巨件玄伉□瘉釐醒
)
//
// 戊件示匹五月    TRUE
//       匹五卅中  FALSE
...
//		BATTLE_Exit( enemyindex, battleindex );
		...
		flg |= CHAR_BATTLEFLG_ULTIMATE;	// 失伙氾奴丟永玄熬仃凶
...
static int BATTLE_PetLoyalCheck( int battleindex, int bid, int charaindex )
{
	...
	mode = 0;	// 懇仄中墊が
	if( ai >= 80 ){	// 鏑擦蘸ㄧㄟ⊙動曉
		mode = PETAI_MODE_NORMAL;	// 濮覆嶽跦
	}else
	if( ai >= 70 ){	// 鏑擦蘸ㄦㄟ⊙動曉
		if( Rand < 10 )mode = PETAI_MODE_TARGETRANDOM;	// 正□必永玄庍
	}else
	if( ai >= 60 ){
		if( Rand < 20 )mode = PETAI_MODE_TARGETRANDOM;	// 正□必永玄庍
	}else
	if( ai >= 50 ){
		if( Rand < 35 )mode = PETAI_MODE_TARGETRANDOM;	// 正□必永玄庍
	}else
	if( ai >= 40 ){
		if( Rand < 50 )mode = PETAI_MODE_TARGETRANDOM;	// 正□必永玄庍
	}else
	if( ai >= 30 ){
		if( Rand < 70 )mode = PETAI_MODE_RANDOMACT;		// 墊が仿件母丞
	}else
	if( ai >= 20 ){
		if( Rand < 70 )mode = PETAI_MODE_RANDOMACT;		// 墊が仿件母丞
	}else
	if( ai >= 10 ){
		if( Rand < 80 ){
			mode = PETAI_MODE_OWNERATTACK;	// 潛諦僎猾
		}else{
			mode = PETAI_MODE_ENEMYATTACK;	// 襯僎猾
		}
	}else{
		if( Rand < 60 ){
			mode = PETAI_MODE_OWNERATTACK;	// 潛諦僎猾
		}else{
			mode = PETAI_MODE_ESCAPE;		// い仆月
		}
	}
	...
	case PETAI_MODE_TARGETRANDOM:
		...
		break;
	case PETAI_MODE_RANDOMACT:	// 墊が互仿件母丞
		// 譁萄ㄠ蔥僎猾蟆反濮覆井尹切扎母丟
	...
	case PETAI_MODE_OWNERATTACK:	// 潛諦僎猾
		CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM1, BATTLE_COM_ATTACK );
		CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, bid-5 );
		break;

	case PETAI_MODE_ENEMYATTACK:	// 襯僎猾
		{ int myside = 0;
	...
	case PETAI_MODE_ESCAPE:		// い仆月公及詨伕旦玄
...
static int BATTLE_Battling( int battleindex )
{
	...
	for( i = 0; i < entrynum; i ++ ){
		...
		case BATTLE_COM_S_MIGHTY:
			...
			gBattleDuckModyfy = CHAR_GETWORKINT_HIGH( charaindex, CHAR_WORKBATTLECOM3 );	// ъ歹今木膜懇騷橘尺
			break;
		}
