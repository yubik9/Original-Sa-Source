...
void ITEM_useRecovery_Battle( int charaindex, int toNo, int haveitemindex )
{
	...
	arg = ITEM_getChar(itemindex, ITEM_ARGUMENT );
#ifdef _CHANGEITEMUSE	 // Syu ADD ½Õ¾ã¾Ô°«¤¤¨Ï¥Î®Æ²z³]©w
	if( ((p = strstr( arg, "Åé" )) != NULL) && ((p = strstr( arg,"®ð")) != NULL))
	{
		kind = BD_KIND_HP_MP;
		p = strstr( arg,"®ð");
	...
	else if( (p = strstr( arg, "Åé" )) != NULL )
#else
	if( (p = strstr( arg, "Åé" )) != NULL )
#endif
	{
		kind = BD_KIND_HP;
	}
	else if( (p = strstr( arg, "®ð" )) != NULL )
	{
		kind = BD_KIND_MP;
	}
	else
#ifdef _ITEM_UNBECOMEPIG
    if( (p = strstr( arg, "¸Ñ½Þ" )) != NULL ){
        kind = BD_KIND_UNBECOMEPIG;
		HealedEffect = 100608; //¯S®Ä½s¸¹
	}
	else
#endif
#ifdef _ITEM_LVUPUP
...
void ITEM_useStatusRecovery_Battle(
	...
	int toNo, 			// »È¤ï¤ì¤ë¿Í¤ÎÈÖ¹æ
	int haveitemindex 	// »È¤¦¿Í¤Î¥¢¥¤¥Æ¥à¤ÎÍóÈÖ¹æ
){
	int i;
...
//--------------------------------------------------------------
//  ¥Õ¥£¡¼¥ë¥ÉÂ°À­ÊÑ¹¹¥¢¥¤¥Æ¥à¤ò»È¤Ã¤¿¾ì¹ç¤Î½èÍý
//--------------------------------------------------------------
// ÀïÆ®Ãæ¤Î¾ì¹ç
...
void ITEM_useCracker_Effect( charaindex, toindex, haveitemindex)
{
	...
	//²M°£¹D¨ã
	BATTLE_ItemUseDelete(charaindex,haveitemindex);
	CHAR_talkToCli(charaindex,-1,"¹D¨ã®ø¥¢¤F¡C",CHAR_COLORYELLOW);
...
//print("\nvincent-->charaindex:%d,attackNo:%d,index2:%d,defNo:%d,",charaindex,attackNo,index2,defNo);
    ...
    /* ¥­¥ã¥é¥¯¥¿¡¼¤Î½ê»ýÉÊ¥ê¥¹¥È¤«¤é¾Ãµî */
    CHAR_setItemIndex(charaindex, haveitemindex ,-1);
	CHAR_sendItemDataOne( charaindex, haveitemindex);/* ¥¢¥¤¥Æ¥à¹¹¿· */
...
void ITEM_useAddexp_Effect( charaindex, toindex, haveitemindex)
{
	...
	//²M°£¹D¨ã
	BATTLE_ItemUseDelete(charaindex,haveitemindex);
	//sprintf(szBuffer, "¾Ç²ß¸gÅçªº¯à¤O´£ª@¤F%d¢H", power);
	sprintf(szBuffer, "¾Ç²ß¸gÅçªº¯à¤O´£ª@¤F%d¢H¡A®É®Ä³Ñ¾l%d¤ÀÄÁ¡C", power, vtime);
...
//®ðµ´ª¬ºA¦^´_­@¤O,½Æ¥[´_¬¡(¥ú,Ãè,¦u)ºëÆF °Ñ¼Æ¦PÅ]ªk(¥ú,Ãè,¦u)ºëÆF,¦ý¥u¯à¦b¾Ô°«¤§¨Ï¥Î,¥Î§¹´N¨S¤F 
//°Ñ¼Æ ¨Ò¦p:­@¤O­È §l turn 1
