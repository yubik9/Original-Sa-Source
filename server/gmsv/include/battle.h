...
{
	BOOL	use;	/* 銀勻化中月井升丹井 */
	int		battleindex; /* 田玄伙�砦� */
	int		mode;	/* 蜇箕及爵お乒□玉 */
	int		type;	/* 爵お正奶皿 (0:騷橘)(1:DUEL)(2:示旦爵) */
	int		dpbattle;	/* DP田玄伙井〝 */
	int		norisk;	/* 韶氏匹手伉旦弁及拑中田玄伙井〝 */
	int		turn;	/* 正□件醒 */
	int		timer;	/* 它巨奶玄羈卞銀丹正奶穴 */
	int		leaderindex; /* 巨件市它件玄毛粟仇仄凶平乓仿及奶件犯永弁旦 */
	int		winside; /* 鴃厙仄凶扔奶玉 */
	int		field_att;	/* 白奴□伙玉及簞嶺 */
	int		att_count;	/* 白奴□伙玉及簞嶺庍祭醱及正奶穴 */
	int		att_pow;	/* 白奴□伙玉及簞嶺庍祭醱及由伐□ */
	int		field_no;
	int		flg;		/* 備潘白仿弘 */
	BATTLE_SIDE		Side[2];
	...
	int enemynum;
#endif
	int		iEntryBack[BATTLE_ENTRY_MAX*2];		// 蟆正□件瓚狤今木化中凶丟件田□
	int		iEntryBack2[BATTLE_ENTRY_MAX*2];		// 蟆正□件瓚狤今木化中凶丟件田□
	int		createindex;	/* 仇及爵お毛綜曰請仄凶平乓仿奶件犯永弁旦(NPC卅升) */
	...
	int battlemap;
#endif
	struct _Battle *pNext;		// 慼及棋爵弘伙□皿
	struct _Battle *pBefore;	// 蟆及棋爵弘伙□皿
}BATTLE;

#define BATTLE_FLG_FREEDP ( 1 << 0 )	// い仆化手�耀邽漱擗吨�
#define BATTLE_FLG_CHARALOST   ( 1 << 1 )	// 平乓仿互韶氏分曰�握惜縣磥楔々中�



enum{	// 曉及field_att 卞ェ木月襖
	BATTLE_ATTR_NONE = 0,	// 拑簞嶺
	BATTLE_ATTR_EARTH,		// 譁
	BATTLE_ATTR_WATER,		// 踹
	BATTLE_ATTR_FIRE,		// 紹
	BATTLE_ATTR_WIND,		// 氘
...
extern int	gItemCrushRate;		// г莽峔犯白巧伙玄
extern BATTLE *BattleArray; 	/* 爵お正旦弁т昫 */
extern int BATTLE_battlenum;	/* т昫及醒 */
extern char szAllBattleString[BATTLE_STRING_MAX];	/* 爵お卞銀丹戊穴件玉妐儂昫 */
extern char *pszBattleTop, *pszBattleLast;	/* 爵お卞銀丹妐儂昫及匏譆 */
extern char szBadStatusString[];	// 旦氾□正旦唱橘迕妐儂昫
extern int gWeponType;	// 蜇箕及朹湛及潘挀
...
#define CH_FIX_PLAYEULTIMATE	(-4)	// 皿伊奶乩□互失伙氾奴丟永玄韶
#define CH_FIX_PETESCAPE		(-1)	// 矢永玄互い仆凶

#define AI_FIX_PETLEVELUP		(+5*100)	// 矢永玄互伊矛伙失永皿
#define AI_FIX_PETWIN			(+1)		// 矢永玄互襯毛邐仄凶
#define AI_FIX_PETGOLDWIN		(+2*10)	// 矢永玄互伊矛伙及嫖中襯毛邐仄凶
#define AI_FIX_PETRECOVERY		(+10)		// 爵お醱卞莢汊仄化手日勻凶
#define AI_FIX_PETRESSURECT		(+3*100)	// 爵お醱卞汊唾仄化手日勻凶
//#define AI_FIX_PETRECOVERY	(+50)		// 爵お醱卞莢汊仄化手日勻凶

#define AI_FIX_SEKKAN 			(-2*100)	// 憤坌及矢永玄毛僎猾
#define AI_FIX_PLAYERULTIMATE	(-10*100)	// 憤坌及潛諦互失伙氾奴丟永玄韶
#define AI_FIX_PETULTIMATE		(-10*100)	// 矢永玄互失伙氾奴丟永玄韶
...
int	BATTLE_PetDefaultEntry(
	int charaindex,	// 矢永玄毛儅勻化中月皿伊奶乩□及�氿峉耀籟�
