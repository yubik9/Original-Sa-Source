 ...
 * 仇木分仃及汔及豳箋及�炾洃羆銴磥忖趥鬗豸吨諢�
 **********************************************/

/* 豳箋及襖庍が及仿件母丞汔及瘉劑午瘉釐﹋ㄠㄟㄟㄟ坌峔﹌ */
#define		ITEM_GEN_RAND_MIN		700
#define		ITEM_GEN_RAND_MAX		1200

/* 失奶氾丞毛腹綢允月豳箋及襖及汔及瘉劑午瘉釐及у峔(double) */
...
static struct _tagItemRandRangeTableForItem {
	int	num;			/* 豳箋及備仿件弁及湘呿襖 */
	int	minnum;			/* 仿件弁醱潸曰丹月襖及Min */
	int	maxnum;			/* 仿件弁醱潸曰丹月襖及MAX﹋num +慼及仿件弁及犒*ITEM_GEN_RATE) */
	double rate;		/* у峔 maxnum / num*/
...
/*
  ...
  70 ~ 130 卞剽域卞坌幵允月﹝午曰丐尹內剽域﹝
  剽域元扎卅仁仄凶曰允月井手仄木卅中及匹﹜
  失奶氾丞燬迕伙□民件午仄化仇仇卞爛聒允月 by ringo 1999Oct1 
...
/*
  ...
  int *fixuse : 襖瑛絆婁醒﹝т昫及瘉釐醒毛午曰﹜爛聒醒毛中木化井尹允

  int *fixatom : 升及豳箋卞覆允月膜懇卅及井
  int *baseup : 湘呿襖卞凶中允月笛遙
  int *minadd : 井仃遙及瘉凝卞凶中允月笛遙
  int *maxadd : 井仃遙及瘉釐卞凶中允月笛遙
		...
		for( i = 0; i < cnt; i ++ ) {
			...
			if( pilenum <= 0 ){
				...
				LogItem(
					...
					itemindexs[i],
#else
					ITEM_getInt( itemindexs[i], ITEM_ID),  /* 失奶氾丞�砦� */
#endif
					"mergedel(合成所刪除的道具)",
