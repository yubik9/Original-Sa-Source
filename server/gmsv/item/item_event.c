...
typedef struct {
    char* cmd;        /* 摹帎妐儂昫 */
    char*   onmessage;    /*  丟永本□斥妐儂昫    */
    char*   offmessage;    /*  丟永本□斥妐儂昫    */
    int   element;    /* 覆擂卞卅月邰豳 */
    int   maxElement; /* element匹隙爛今木凶邰豳及瘉釐襖毛手勾邰豳 */
} ITEM_EFFECTPARAM;
...
{
	int battlemode;
	if( CHAR_CHECKINDEX( charaindex ) == FALSE )return ; //﹛撩л
	battlemode = CHAR_getWorkInt( charaindex, CHAR_WORKBATTLEMODE );
	if( battlemode == BATTLE_CHARMODE_INIT ){
	}else
	if( battlemode ){
...
//-------------------------------------------------------------------------
//	今中仇欠毛撢蜇允月楮醒﹝
//	侹午仄凶凜卞﹞仿件母丞卞ㄠ℅ㄥ毛薊太﹞抩蟆午脰颶�砦璊藡揤馱馱諢�
//-------------------------------------------------------------------------
...
void ITEM_useLottery( int charaindex, int toindex, int haveitemindex)
{
	...
	// ㄥ莢杴及銀迕反﹞陸木仄井丐曰ぜ卅中及匹﹞仇仇匹失奶氾丞毛壅允
	else if( count == 6 ) {
        CHAR_setItemIndex( charaindex , haveitemindex, -1 );
		CHAR_sendItemDataOne( charaindex, haveitemindex);
        ITEM_endExistItemsOne( itemindex );
	...
	// for debug
#if 0
    snprintf( buff, sizeof( buff), "羈操“%d 請杴: %d%d%d%d%d%d",
    			hit,
    			(int)(n[0]+1),(int)(n[1]+1),
    			(int)(n[2]+1),(int)(n[3]+1),
    			(int)(n[4]+1),(int)(n[5]+1)
...
//                            CHAR_COLORWHITE );
	...
	// 公丹元扎拑中凜反﹞犯□正毛霜曰卅云允分仃﹝
	else {
		char strbuff[1024];
		if( flg) {
			// 旦矢□旦煌遙
			// 失奶氾丞及濩抸勂及戊丟件玄反﹞ㄠ墊ㄡㄧ妐儂匹﹞
			// 旦矢□旦毛銀勻化荼墊仄化中月﹝
...
print("\nvincent-->charaindex:%d,toindex:%d",charaindex,toindex);
    ...
    /* 平乓仿弁正□及赭儅圴伉旦玄井日壅蛔 */
    CHAR_setItemIndex(charaindex, haveitemindex ,-1);
	CHAR_sendItemDataOne( charaindex, haveitemindex);/* 失奶氾丞凳蕙 */
	/* 壅允 */
	ITEM_endExistItemsOne( itemindex );
}
#endif
