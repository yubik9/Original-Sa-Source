...
typedef struct tag_serverState
{
    ...
    unsigned int    closeallsocketnum;  /*   closeallsocket 醱及酸曰及
                                             醒*/
	int				shutdown;		/*  扔□田毛shutdown允月乒□玉 
									 *	0:騷橘 公木動陸:扑乓永玄扑乓永玄乒□玉
									 * 乒□玉卞卅勻凶凜棉互ェ勻化月﹝
									 */
	int				dsptime;		/* shutdown 乒□玉及伐□弁庍醒*/
	int				limittime;		/* 仇木手 */
}ServerState;
...
#define MTIO_FIXED_BUFSIZE (65536)
    ...
    int wbuse;
    pthread_mutex_t mutex;    /* Connect毛中元月午五卞勾井丹伕永弁 */
#else
    char *rb;
    int rbuse;
    char *wb;
    ...
    LoginType state;        /* 蜇箕及伕弘奶件橇謫 */
	int		nstatecount;
    char    charname[CHARNAMELEN];  /* 伕弘奶件醱及平乓仿抩 */
    int     charaindex;     /* charт昫尺及奶件犯永弁旦﹝
                             * 伕弘奶件詨卞襖互澀爛今木月﹝-1互犯白巧伙玄
                             * т昫卞卅中凜﹝
                             */
    char    CAbuf[2048];         /*  CA() 毛做諒允月啃及田永白央 */
    int     CAbufsiz;       /*  CAbuf 及扔奶朮  */
    struct timeval  lastCAsendtime;     /*瘉詨卞CA毛霜勻凶凜棉 */

    char    CDbuf[2048];         /*  CD() 毛做諒允月啃及田永白央 */
    int     CDbufsiz;       /*  CDbuf 及扔奶朮  */
    struct timeval  lastCDsendtime;     /*瘉詨卞CD毛霜勻凶凜棉 */

    struct timeval  lastCharSaveTime; /* 瘉詨卞平乓仿犯□正毛本□皮仄凶凜棉 */
    
    struct timeval  lastprocesstime;    /* 瘉詨卞皿伕玄戊伙毛質咥仄凶凜棉*/
...
//	unsigned int		DefBtime;
    ...
    int appendwb_overflow_flag;  /* 1莢匹手appendWb互撩л仄凶日1卞允月 */
...
#if USE_MTIO
pthread_mutex_t MTIO_servstate_m;     /* servstate 及伕永弁 */
...
/* 曉筏及伕弘反戶切扎仁切扎犯奴旦弁毛罹丹及匹旦伉□皿毛中木凶曰 */
 ...
 *  fdid    int     fd及id
 * 忒曰襖
 *  -1 及凜反伕弘奶件醱及平乓仿反心勾井日卅井勻凶﹝0動曉卅日
 * 伕弘奶件醱及平乓仿及平乓仿反中木勾尺及 index
 ------------------------------------------------------------*/
...
/*
  1墊午曰分允﹝午曰分仄凶卅互今毛井尹允
  嫖簧祭及凶戶﹜ outlen 卞反勾友卞蝸坌卅扔奶朮互反中勻化月午移爛允月冗
 */
