...
}
{
		...
		/* ¤â¤·¤â£²ÅÙ¤á¤Î¥ë¡¼¥×¤ËÆþ¤Ã¤¿»þ¤Î¤¿¤á¤Ë½é´ü²½¤·¤Æ¤ª¤¯ */
    	for( j = 0; j < GROUP_DATAINTNUM; j ++ ) {
...
static int ENEMY_getExp( int array,int tarray, int level, int rank )
{
	...
	alpha = (   *( tp + E_T_CRITICAL) + *( tp + E_T_COUNTER)   + *(tp + E_T_GET)
			...
			+ *( tp + E_T_RARE);
	/* EXP¡á´ðËÜEXP¡Ü¡Ð¡Ê¥é¥ó¥¯¡Ü¦Á¡Ë¡ö¥ì¥Ù¥ë¡Ñ*/
...
static int EnemyGymSkill[] = {
	PETSKILL_GUARDBREAK, 			// £³¡§¥¬¡¼¥ÉÊø¤·(3)
	PETSKILL_CONTINUATIONATTACK1, 	// £±£°¡§Ï¢Â³¹¶·â(10)
	PETSKILL_CONTINUATIONATTACK2, 	// £±£±¡§Ï¢Â³¹¶·â(11)
	PETSKILL_CONTINUATIONATTACK3, 	// £±£²¡§Ï¢Â³¹¶·â(12)
	PETSKILL_CHARGE1,				// £³£°¡§¥Á¥ã¡¼¥¸£±(30)
	PETSKILL_CHARGE2,				// £³£±¡§¥Á¥ã¡¼¥¸£²(31)
	PETSKILL_MIGHTY1,				// £´£°¡§°ì·âÉ¬»¦(40)
	PETSKILL_MIGHTY2,				// £´£±¡§°ì·âÉ¬»¦(41)
	PETSKILL_POWERBALANCE1,			// £µ£°¡§ÇØ¿å¤Î¿Ø£±(50)
	PETSKILL_POWERBALANCE2,			// £µ£±¡§ÇØ¿å¤Î¿Ø£²(51)
	PETSKILL_POWERBALANCE3,			// £µ£²¡§ÇØ¿å¤Î¿Ø£³(52)
	PETSKILL_POISON_ATTACK1,		// £¶£°¡§ÆÇ¹¶·â(60)
	PETSKILL_POISON_ATTACK2,		// £¶£±¡§ÆÇ¹¶·â(61)
	PETSKILL_STONE,					// £¸£°¡§ÀÐ²½¹¶·â(80)
	PETSKILL_CONFUSION_ATTACK,		// £¹£°¡§º®Íð¹¶·â(90)
	PETSKILL_DRUNK_ATTACK,			// £±£°£°¡§Å¥¿ì¹¶·â(100)
	PETSKILL_SLEEP_ATTACK,			// £±£±£°¡§Ì²¤ê¹¶·â(110)
	PETSKILL_NOGUARD1,				// £±£µ£°¡§¥Î¡¼¥¬¡¼¥É£±(150)
	PETSKILL_NOGUARD2,				// £±£µ£±¡§¥Î¡¼¥¬¡¼¥É£²(151)
	PETSKILL_NOGUARD3,				// £±£µ£²¡§¥Î¡¼¥¬¡¼¥É£³(152)
...
/* ¥Ñ¥é¥á¡¼¥¿»Ø¿ô¤«¤é¼ÂºÝ¤Î´ðËÜ¥Ñ¥é¥á¡¼¥¿¤Ø¤Î·×»»¼° */
#if 1
...
/*------------------------------------------------------------
 * Å¨ËÜÂÎÀßÄê¤Î¼±ÊÌÈÖ¹æ¤¬ÆÃ¼ì¤ÊÃÍ¤Ê¤é¥é¥ó¥À¥à¤ËÀßÄê¤¹¤ë
 ------------------------------------------------------------*/
...
int PET_CheckIncubate( int charaindex)
{
	...
	for(i=0 ; i < CHAR_MAXPETHAVE ; i++)	{
		...
		if( (int)checkeage > (time_l+(anhour*2)) ){
			...
			char buf[256];
			sprintf( buf, "³J¡q%s¡r§e²{¥iÁý­¹ª¬ºA¡C", CHAR_getUseName( petindex ));
			CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		}
	}
	return 1;
		...
		if( (int)checkeage > (time_l+(anhour*2.5)) ){
			...
			char buf[256];
			sprintf( buf, "³J¡q%s¡r§e²{¥iÁý­¹ª¬ºA¡C", CHAR_getUseName( charaindex ));
			CHAR_talkToCli( masterindex, -1, buf, CHAR_COLORYELLOW);
		}
		return 1;
#endif
