...
void CHAR_JoinParty_Main( int charaindex, int targetindex)
{
	...
	/* Áê¼ê¥Ñ¡¼¥Æ¥£¤Î¿Í¿ô¤Ï£Ï£Ë¤«¡© */
	parray = CHAR_getEmptyPartyArray( toindex) ;
	if( parray == -1 ) {
		print( "%s : %d err\n", __FILE__,__LINE__);
		return;
	}
	/* ²¿¤âÌµ¤·¢ª¿Æ¤Î»þ¤Ï¿Æ¤Ë¤Ê¤Ã¤¿CA¤òÁ÷¿®¤¹¤ë */
...
/*------------------------------------------------------------
 * Ãç´Ö¤ò½çÈÖ¡ÊCHAR_WORKPARTYINDEX)»ØÄê¤Ç¥­¥ã¥éindex¤ò°ú¤ÃÄ¥¤ë¡£
 * ¼«Ê¬¤¬¿Æ¤Ç¤â»Ò¤Ç¤â£Ï£Ë¡£
 ------------------------------------------------------------*/
