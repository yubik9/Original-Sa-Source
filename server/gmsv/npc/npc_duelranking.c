...
#ifdef _DEATH_FAMILY_GM_COMMAND	// WON ADD Æa±⁄æ‘GM´¸•O
#include "family.h"
#endif
...
/*	case 3:
		{
			...
			for( i=0; i<pagecount; i++ ){
				...
				strcpy( buf2, "°–");
				if( battle>= 30 && score > 0 ) strcpy( buf2, "°π");
	...
	case 3://•ø¶°¡…µ{
		{
			...
			case 3:
				...
				break;
			case 5:
				snprintf( token, sizeof( token),	"  ∂§•Ó¶W≥Ê&¡…µ{¨ˆø˝\n\n"
													"¨ˆø˝  \n\n" );
			...
			for( i=0; i<pagecount; i++ ){
				...
				case 3:
					...
					sprintf( buf1, "≥ı%2d %-12s VS %-12s %2d§¿%2d¨Ì %s \n", tindex,
...
/*	case 6://•[§J¡…µ{
		{
			...
			if( CHAR_getInt( toindex, CHAR_PKLISTLEADER) != 1 ) {
				CHAR_talkToCli( toindex, -1, "•u¶≥∂§™¯•i•H•N™Ì∂§•Ó•[§J¡…µ{°C", CHAR_COLORYELLOW);
				return;
			}
			if( PKLIST_JoinPKProcedures( toindex) == FALSE ){
				CHAR_talkToCli( toindex, -1, "•[§J¡…µ{•¢±—°I°C", CHAR_COLORYELLOW);
			}
...
BOOL NPC_CheckJoin_FamilyPK_List( int meindex, int toindex)
{
	...
	print( "check[%d]: [%d,%s,%s]\n",
		...
		if( JoinFamilyList[i].join == 1 ){
			CHAR_talkToCli( toindex, -1, "§w∏g≥¯¶W∞—•[§F°C", CHAR_COLORYELLOW);
			return FALSE;
		}else{
			CHAR_talkToCli( toindex, -1, "≥¯¶Wßπ¶®°C", CHAR_COLORYELLOW);
			JoinFamilyList[i].join = 1;
			NPC_BackupFamilyPK_List();
			return TRUE;
		}
	}
	CHAR_talkToCli( toindex, -1, "µL™k≥¯¶W°AΩ–ΩTª{ßA¨Oß_¨∞∏ÍÆÊ¶W≥Ê§W§ßÆa±⁄±⁄™¯°C", CHAR_COLORYELLOW);
	return FALSE;
}
//CHAR_FMINDEX 
#endif
