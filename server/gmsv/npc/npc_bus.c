...
enum {
	NPC_WORK_ROUTETOX = CHAR_NPCWORKINT1,		/* ¤É¤³¤Ø¡©£øºÂÉ¸ */
	NPC_WORK_ROUTETOY = CHAR_NPCWORKINT2,		/* ¤É¤³¤Ø¡©£ùºÂÉ¸ */
	NPC_WORK_ROUTEPOINT = CHAR_NPCWORKINT3,		/* º£²¿ÈÖÌÜ¤« */
	NPC_WORK_ROUNDTRIP = CHAR_NPCWORKINT4,		/* ¹Ô¤­¤«µ¢¤ê¤«¡Ê£°¡§¹Ô¤­ £±¡§µ¢¤ê¡Ë*/
...
void NPC_BusLoop( int meindex)
{
	...
	switch( CHAR_getWorkInt( meindex, NPC_WORK_MODE )) {
	  ...
	  case 2:
		...
		return;
	  case 3:
		/* ÅþÃå¤·¤Æ¤â¡¤¥¯¥é¥¤¥¢¥ó¥È¤ÎÉÁ¼ÌÂÔ¤Á¤Î°Ù¤Ë¡¤
		 * ¾¯¤·¤³¤³¤Ç¥¦¥§¥¤¥È¤ò¤¤¤ì¤Æ¤ä¤ë
		 */
		...
		if( ret == CHAR_WALKSUCCESSED ) {
			...
			for( i = 1; i < CHAR_PARTYMAX; i ++ ) {
				...
				if( CHAR_CHECKINDEX(toindex) ) {
					int		parent_dir;
					/* »Ò¤Î°ÌÃÖ¤È¡¤¿Æ¤ÎÊâ¤­Á°¤Î°ÌÃÖ¤«¤éÊý¸þ¤òµá¤á¤ë */
					/* Êâ¤¯ */
					start.x = CHAR_getInt( toindex, CHAR_X);
					start.y = CHAR_getInt( toindex, CHAR_Y);
					parent_dir = NPC_Util_getDirFromTwoPoint( &start,&end );
					/* ¥°¥é¥Ç¥£¥¦¥¹¥ª¥×¥·¥ç¥óÊâ¤­¤ò¼Â¸½¤¹¤ë°Ù¤Ë¡¤
					 * ¼¡¤Î»Ò¤ÏÁ°¤Î»Ò¤Î¸å¤òÄÉ¤¦¤è¤¦¤Ë¤¹¤ë
					 */
					end = start;
					if( parent_dir != -1 ) {
...
//		return FALSE;
//	}
	/* ¸æ¶â¤Î¥Á¥§¥Ã¥¯¤ò¤¹¤ë¡Ê¤ª¶â¤ò¼è¤ë¤Î¤Ç¡¤ºÇ½ª¥Á¥§¥Ã¥¯¤Ë¤¹¤ë¤³¤È¡ª¡Ë */
