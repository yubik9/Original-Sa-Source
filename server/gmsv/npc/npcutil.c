...
// Arminius: §Ú§âÁn±æ­È´î¥b
int FMAdvTbl[] = {	// ®a±Ú«_ÀI Table
	0,	// 0
	15,	// 1
	5,	// 2
	0,	// 3
...
BOOL NPC_Util_isFaceToChara( int index1, int index2, int distance )
{
    ...
    for( i=1;i<=distance;i++ ){
        /* ¤Þ¤º¡¢¤¤¤Ã¤³¤á¤Î¥­¥ã¥é¤¬Êâ¤¤¤¿¸å¤ÎºÂÉ¸¤òµá¤á¤ë */
        int walked_x, walked_y;
        CHAR_getCoordinationDir( CHAR_getInt(index1,CHAR_DIR)
                                 , CHAR_getInt(index1,CHAR_X)
                                 , CHAR_getInt(index1,CHAR_Y)
...
static int SearchNearAround(
	int x, 	   /* ¸¡º÷Ãæ¿´¤Î£ØºÂÉ¸ */
	int y,     /* ¸¡º÷Ãæ¿´¤Î£ÙºÂÉ¸ */
	int floor, /* ¸¡º÷Ãæ¿´¤Î¥Õ¥í¥¢ */
	int Part,  /* ¸¡º÷³«»Ï¤¹¤ëºÇ½é¤Î¥é¥¤¥ó */
	int Level, /* Ãæ¿´¤è¤êÎ¥¤ì¤Æ¤¤¤ëµ÷Î¥ */
	int type   /* ¸¡º÷¤¹¤ë CHAR ¥¿¥¤¥× */
){
	int i, iTarget = -1;
	for( i = 0; i < 4; i ++ ){
		if( Part == 0 ){ /* ¾å¤Î¥é¥¤¥ó¤ò¸¡º÷ */
		...
		if( Part == 3 ){ /* º¸¤Î¥é¥¤¥ó¤ò¸¡º÷ */
			iTarget = SearchNearLine( x-Level, y+Level-1,
				floor, 0, -1, Level*2, type );
			if( iTarget >= 0 ) break;
		}
		Part ++; Part &= 3; /* ¼¡¤Î¥Ñ¡¼¥È¤ò¸¡º÷ */
	}
	return iTarget;
}
...
int NPC_Util_SearchNear( int meindex, int maxlen, int type )
{
    ...
    for( i = 0 ; i<objnum ; i++ ){
		...
		if( iMin > iLen ){
			iMin = iLen;
			iTarget = obj[i].index;	/* ¥¤¥ó¥Ç¥Ã¥¯¥¹¤òÅÐÏ¿*/
		}
	}
...
/*******************************************************
  ...
  ¼Ð¤á°ÜÆ°¤¹¤ëÁ°¤Ë²¿¤«¤Ë¤Ö¤Ä¤«¤Ã¤Æ¤¤¤ë¤«¤É¤¦¤«Ä´¤Ù¤ë
  int NPC_Util_SuberiWalk(
	int	index,	CHAR ¤Î¥¤¥ó¥Ç¥Ã¥¯¥¹
	int	dir		Êâ¤¯¸þ¤­
  );

  Ìá¤êÃÍ  0¡Á7¡§Êâ¤±¤ëÊý¸þ
          -1  ¡§¥Þ¥Ã¥×¤Ë°ú¤Ã¤«¤«¤Ã¤Æ¥¨¥é¡¼
          -2  ¡§¥­¥ã¥é¥¯¥¿¤Ë°ú¤Ã¤«¤«¤Ã¤Æ¥¨¥é¡¼
          -3  ¡§Î¾Êý¤Ë°ú¤Ã¤«¤«¤Ã¤Æ¥¨¥é¡¼

  ¼Ð¤á°ÜÆ°¤¹¤ë¾ì¹ç¤Þ¤º½Ä¡¢²£¤Ë°ÜÆ°¤Ç¤­¤ë¤«Ä´¤Ù¤ë¡£
  Î¾Êý°ÜÆ°¤Ç¤­¤ì¤Ð¼Ð¤á¤ò¥Á¥§¥Ã¥¯¤¹¤ë¡£ÊÒÊý¤·¤«°ÜÆ°¤Ç¤­¤Ê¤±¤ì¤Ð
  ¤½¤Á¤é¤ÎÊý¸þ¤òÊÖ¤¹¡£
...
/*		CHAR_walk( index, dir, 0 );*/
	...
	for( i = 0; i < 3; i ++ ){
		...
		if( MAP_walkAble( index,fl,tX,tY ) == 0 ){
			aDirList[i] = -1; /* ¹Ô¤±¤Ê¤¤¾ì¹ç¤Ï¤½¤ÎÊý¸þËõ»¦*/
//		}else if( NPC_Util_OtherCharCheck( fl,tX, tY ) ){
//			aDirList[i] = -2; /* ¹Ô¤±¤Ê¤¤¾ì¹ç¤Ï¤½¤ÎÊý¸þËõ»¦*/
		}else{
			/* ¹Ô¤±¤¿!!*/
			OkFlg ++;
		}
...
/*-----------------------------------------------------------------------
* NPC¤Îcreate»þ¤Î°ú¿ô¤ÎÃæ¤«¤é»ØÄê¤µ¤ì¤¿Ê¸»úÎó¤ò¸¡º÷¤·¡¤
* ¤½¤ÎÃæ¤Î¿ôÃÍ¤ò¼èÆÀ¤¹¤ë¡£¥ª¥×¥·¥ç¥ó¤Ç¿ôÃÍ¤ò»ØÄê¤µ¤»¤Æ¤ä¤ê¤¿¤¤¤È¤­¤ËÊØÍø
*
* Ãí°Õ¡§¥Ç¥ê¥ß¥¿¤Ï"|"¤Ë¸ÂÄê¤·¤Æ¤Þ¤¹¡£
*       °ú¿ô¤ËÆ±¤¸Ê¸»úÎó¤¬£²¤ÄÊÂ¤ó¤À¾ì¹ç¡¤
*       Àè¤ÎÊý¤¬Í¥Àè¤µ¤ì¤Þ¤¹¡£
* ----------------------------------
* IN : int meindex : ¥­¥ã¥éindex
*    : char* in    : ¸¡º÷¤¹¤ëÊ¸»úÎó
* OUT:
* RETURN:
* ¼èÆÀ½ÐÍè¤Ê¤«¤Ã¤¿»þ :-1
...
/*
 * ¥¢¥¤¥Æ¥à¤Î index ¤È¥­¥ã¥é index ¤Î¤Õ¤¿¤Ä¤Î¾ðÊó¤«¤é¡¢
 * ¥­¥ã¥é¤¬¤½¤Î¥¢¥¤¥Æ¥à¤ò»ý¤Ã¤Æ¤ë¤«¤É¤¦¤«¤·¤é¤Ù¤Æ»ý¤Ã¤Æ¤¿¤é¤É¤Î
 * °ÌÃÖ¤Ë»ý¤Ã¤Æ¤ë¤«¤òÊÖ¤¹¡£  ¥¢¥¤¥Æ¥à´ØÏ¢¤ÎNPCÍÑ¡£CHAR_¤Ë°Ü¹Ô
 * ¤Ç¤­¤ë¤«¤â¤·¤ì¤ó´Ø¿ô¥Ê¥ê
 * by ringo
 *
 * int charindex : ÂÐ¾Ý¥­¥ã¥é¤Î index
 * int itemindex : ¥¢¥¤¥Æ¥à¤Îindex
 *
 * ÊÖ¤êÃÍ
 *
 *  >=0 : ¤É¤Î°ÌÃÖ¤Ë»ý¤Ã¤Æ¤¤¤ë¤Î¤«¡£Í­¸ú¤ÊÃÍ¡£
 *  -1 : ¤â¤·¤¯¤Ï»ý¤Ã¤Æ¤Ê¤¤¡£
 *  -2 : ¥¨¥é¡¼¡£
 */
...
/*------------------------------------------------------------------------
* ¥­¥ã¥é¤«¤é¤ß¤¿Â¾¤Î¥­¥ã¥é¤ÎÊý¸þ¤òµá¤á¤ë
*
* int NPC_Util_GetDirCharToChar( int fromindex, int toindex, int mode)
*
* int	fromindex 	¤³¤Î¥­¥ã¥éindex¤«¤é
* int	toindex		¤³¤Î¥­¥ã¥éindex¤ËÂÐ¤·¤Æ¤ÎÊý¸þ¤¬µá¤á¤é¤ì¤ë
* int	mode		¼èÆÀ¤¹¤ëÊý¸þ
*                   0:toindex¤Ë¸þ¤«¤Ã¤Æ
*                   1:toindex¤ÎÈ¿ÂÐÊý¸þ
*                   2:toindex¤ËÂÐ¤·¤Æ±¦£¹£°ÅÙÊý¸þ
*                   3:toindex¤ËÂÐ¤·¤Æº¸£¹£°ÅÙÊý¸þ
*
* ÊÖ¤êÃÍ À®¸ù ¡§dir¡ÊÊý¸þ¤òÊÖ¤¹¡Ë
*        ¼ºÇÔ ¡§-1
------------------------------------------------------------------------*/
int NPC_Util_GetDirCharToChar( int fromindex, int toindex, int mode)
{
...
/*------------------------------------------------------------------------
* ÌÜÉ¸¥­¥ã¥é¤ËÂÐ¤·¤Æ¡¤¤¢¤ëÊý¸þ¤Ë°ÜÆ°¤¹¤ë¡£
*
* int NPC_Util_WalkCharToChar( int fromindex, int toindex, int mode)
*
* int	fromindex 	Êâ¤¯¥­¥ã¥éindex
* int	toindex		Êâ¤¯ÌÜÉ¸¤Î¥­¥ã¥éindex
* int	mode		Êâ¤¯Êý¸þ
*                   0:toindex¤Ë¸þ¤«¤Ã¤Æ
*                   1:toindex¤ÎÈ¿ÂÐÊý¸þ
*                   2:toindex¤ËÂÐ¤·¤Æ±¦£¹£°ÅÙÊý¸þ
*                   3:toindex¤ËÂÐ¤·¤Æº¸£¹£°ÅÙÊý¸þ
* BOOL	suberi		NPC_Util_SuberiWalk¤ò¹ÍÎ¸¤¹¤ë¤«¡©TRUE:¤¹¤ë FALSE:¤·¤Ê¤¤
*
* ÊÖ¤êÃÍ À®¸ù¡ÊÊâ¤¤¤¿¡Ë¡§dir¡ÊÊâ¤¤¤¿Êý¸þ¤òÊÖ¤¹¡Ë
*        ¼ºÇÔ          ¡§-1
------------------------------------------------------------------------*/
...
void NPC_Util_NPCDelete( int srcindex)
{
	...
	/* createnum ¤¬ºÇÂç¤Î»þ¡¤À¸¤Þ¤ì¤¿»þ´Ö¤ò¥»¥Ã¥È¤¹¤ë
	 * ¤Ê¤¼¤³¤³¤Ç¤³¤ó¤Ê»ö¤ò¤·¤Æ¤¤¤ë¤«¤È¤¤¤¦¤È
	 * create¥Õ¥¡¥¤¥ë¤ÎtimeÃÍ¤¬À¸¤Þ¤ì¤Æ¤«¤é¤½¤ì¤À¤±Ê¬·Ð¤Ã¤Æ¤¤¤ë¤È¡¤
	 * ÀäÂÐ¤Ë¤¹¤°¤ËÀ¸¤Þ¤ì¤Æ¤¯¤ë¤È¤¤¤¦»ÅÍÍ¤ò¤Ê¤ó¤È¤«¤·¤è¤¦¤È¤¹¤ë°Ù
	 */
		...
		if(strlen(CHAR_getChar(talker,CHAR_TEACHER_ID)) > 0 && strlen(CHAR_getChar(talker,CHAR_TEACHER_NAME)) > 0){
			...
			for(i=0;i<iPlayernum;i++){
				...
				if(strcmp(CHAR_getChar(talker,CHAR_TEACHER_ID),CHAR_getChar(i,CHAR_CDKEY)) == 0 &&
					...
					sprintf(szMsg,"Àò±o¾Ç¥Í %s %.2f ÂIÁn±æ",CHAR_getChar(talker,CHAR_NAME),fGetFame);
	   ...
	   int fd = getfdFromCharaIndex(talker);
...
#ifdef _NEW_MANOR_LAW
		 {
			 ...
			 for(i=0;i<FAMILY_MAXHOME;i++){
				 // ¥ô¦ó¤@­Ó²ø¶é¦³¦b¬D¾Ô±Æµ{,®ð¶Õ­È¤@§ïÅÜ´N¦VAC­n³Ì·s®ð¶Õ¸ê®Æ
			 ...
			 for( i=0 ; i<FMPOINTNUM ; i++ ) {
				...
				if( village == npc_village ) {
					if( getStringFromIndexWithDelim(fmpointlist.pointlistarray[i], "|", 5, token, sizeof(token)) == FALSE)
						return 0;
					fmindex = atoi(token);
// Terry add for ¦pªG²ø¶é¨S¤H¦û»â«h¤£­pºâ
