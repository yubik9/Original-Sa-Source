...
void NPC_FmLetterTalked( int meindex , int talkerindex , char *szMes ,int color )
{
	...
	if( CHAR_getInt( talkerindex, CHAR_FMLEADERFLAG ) == 1
		...
		lssproto_WN_send( getfdFromCharaIndex( talkerindex ), WINDOW_MESSAGETYPE_MESSAGE,
			...
			-1, -1,
			makeEscapeString( "\n¥u¦³¥»²ø¶éªº±Úªø¤~¯à»s§@ÁÜ½Ð¨ç¤Î¦P·N®Ñ¡I", buf, sizeof(buf)));
	...
	if( buttonproc[button].use == TRUE ) {
		...
		if( newwin == 5 )	{
			...
			if( itemindex != -1 ){
				...
				LogItem(
					CHAR_getChar( talkerindex, CHAR_NAME ), /* ¥­¥ã¥éÌ¾ */
...
static BOOL NPC_FmLetter_readData( int meindex, int windowno, BOOL chkflg)
{
	...
	while( readflg == TRUE ) {
		...
		while( 1) {
			...
			if( winno == -1 ) {
				print( "fmletter:winno ©|¥¼©w¸q¡A¸ê®Æ«o¤w³]©w¡C\n");
				print( "filename:[%s] line[%d]\n", filename, linenum);
				readflg = FALSE;
				errflg = FALSE;
				break;
		...
		if( winno != -1 ) {
			if( w.windowtype == -1 ) {
				print( "fmletter: §ä¤£¨ìwintype\n");
				print( "filename:[%s] line[%d]\n", filename, linenum);
				errflg = TRUE;
				break;
			}
			if( w.buttontype == -1 ) {
				print( "fmletter: §ä¤£¨ìbutton\n");
				print( "filename:[%s] line[%d]\n", filename, linenum);
				errflg = TRUE;
				break;
			}
			if( strlen( w.message) == 0 ) {
				print( "fmletter: §ä¤£¨ìmessage\n");
				print( "filename:[%s] line[%d]\n", filename, linenum);
				errflg = TRUE;
				break;
			}
	...
	if( chkflg == FALSE && w.windowno == -1 ) {
		print( "fmletter: §ä¤£¨ì©Ò«ü©wªºwindowno\n");
		print( "filename:[%s] line[%d]\n", filename, linenum);
		return FALSE;
	}
	if( winno != -1 && endflg == FALSE) {
		print( "fmletter: §ä¤£¨ìendwin\n");
		print( "filename:[%s] line[%d]\n", filename, linenum);
		return FALSE;
	}
