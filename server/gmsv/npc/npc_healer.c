	...
	|| (CHAR_getWorkInt( talker, CHAR_WORKPARTYMODE) == 2) )
	{
	    ...
	    if(msgNo == 1) {
		    CHAR_talkToCli( talker, meindex,
			    "¤w¸g¥þ³¡¦^´_¡C½Ð¦b¤U¦¸ªº¤ñÁÉ¤¤¥[ªo­ò¡I",CHAR_COLORWHITE);

		}else if(msgNo == 2) {
		    CHAR_talkToCli( talker, meindex,
		    	"¥Ñ©ó§A«Ü¸Û¹ê¡AÅý§ÚÀ°§A¦^´_§a¡I",CHAR_COLORWHITE);
		}
...
void NPC_HealerAllHeal( int talker )
{
	...
	if(CHAR_getWorkInt( talker, CHAR_WORKPARTYMODE) != CHAR_PARTY_NONE )
	{
		...
		if( CHAR_CHECKINDEX( oyaindex )) {
			...
			/* ¼«Ê¬¤È¼£¤·¤¿¤ä¤Ä¤ÎÃç´Ö¤ÎÇÛÎó¤Î¾ì½ê¤ò¼èÆÀ */
			for( i = 0; i < CHAR_PARTYMAX; i ++ ) {
