...
/*
 *­¡¡¡¥Ü¥Ç¥£¥é¥ó¥²¡¼¥¸¤ò»È¤¨¤ÐÈ¿±þ¤¹¤ëNPC
 * ¤Þ¤ºÏÃ¤·¤«¤±¤é¤ì¤¿¤é¡¢¤½¤Î¥×¥ì¥¤¥ä¡¼¤Ë
 * ¤³¤ÎNPC¤ÎINDEX¤òÊÝÂ¸¤¹¤ë¡£
 *
 *­¢¡¡NPC¤Î¼þ¤ê¤Ç¥¢¥¯¥·¥ç¥ó¤¬¤¢¤Ã¤¿¤é¡¢¥¢¥¯¥·¥ç¥ó¤òµ¯¤³¤·¤¿¥×¥ì¥¤¥ä¡¼¤¬
 *NPC¤ÎINDEX¤òÊÝ»ý¤·¤Æ¤¤¤ë¤«¥Á¥§¥Ã¥¯
 *¥Á¥§¥Ã¥¯£Ï£Ë¤Ê¤é¼¡¤Ï¥¢¥¯¥·¥ç¥ó¤Î¥·¡¼¥±¥ó¥¹¥Á¥§¥Ã¥¯¤¹¤ë
 *´õË¾¤Î¥¢¥¯¥·¥ç¥ó¤ò¤·¤Æ¤¤¤¿¤é¥×¥ì¥¤¥ä¡¼¤Î¥«¥¦¥ó¥È¤ò¥¢¥Ã¥×¤µ¤»¤ë¡£
 *¥«¥¦¥ó¥È¥¢¥Ã¥×¸å¡¢Á´¥·¡¼¥±¥ó¥¹£Ï£Ë¤Ê¤é»ØÄê¤Î¾ì½ê¤Ø¥ï¡¼¥×¤µ¤»¤ë
 *
 */

enum {
	BODYLAN_E_COMMANDNUM = CHAR_NPCWORKINT1,	// ¥³¥Þ¥ó¥É¤ÎÄ¹¤µ
};
...
static void NPC_BodyLan_Profit( int meindex, int playerindex )
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( szArg, "Warp", szP, sizeof( szP ) ) != NULL ){
		...
		if( sscanf( szP, "%d,%d,%d", &fl, &x, &y ) == 3 ){
			...
			print( "npc_bodylan: µLªkÅª¨úªÅ¶¡®y¼Ð(%s)\n", szP );
			return;
		}
		// ¥Ñ¡¼¥Æ¥£ÁÈ¤ó¤Ç¤ë¾ì¹ç»Ò¶¡¤â¥ï¡¼¥×
...
)
{
	...
	switch( mode ){
	  ...
	  case BODYLAN_WIN_LAST_GOOD:
		...
		if( NPC_Util_GetStrFromStrWithDelim( szArg, "Good", szP, sizeof( szP ) ) == NULL ){
			print( "npc_bodylan:µª¹ï®É»¡ªº¤å¦r¨S¦³¿é¤J(%s)\n",	szArg );
			return;
		}
		sprintf( token,"%s", szP );
	  ...
	  case BODYLAN_WIN_LAST_NG:
		...
		if( NPC_Util_GetStrFromStrWithDelim( szArg, "Ng", szP, sizeof( szP ) ) == NULL ){
			print( "npc_bodylan:µª¿ù®É»¡ªº¤å¦r¨S¦³¿é¤J(%s)\n",	szArg );
			return;
		}
		sprintf( token,"%s", szP );
	  ...
	  case BODYLAN_WIN_ALREADY:
		// ¤¹¤Ç¤Ë¥¤¥Ù¥ó¥È¤ò½ªÎ»¤·¤Æ¤¤¤¿¾ì¹ç
		if( NPC_Util_GetStrFromStrWithDelim( szArg, "Good_No", szP, sizeof( szP ) ) == NULL ){
			print( "npc_bodylan:¨Æ¥óµ²§ô®Éªº¤å¦r¨S¦³¿é¤J(%s)\n",	szArg );
			return;
		}
		sprintf( token,"%s", szP );
		buttontype=WINDOW_BUTTONTYPE_OK;	// OK
	...
	case BODYLAN_WIN_NOT_PREEVENT:
		// »öÁ°¤ËÉ¬Í×¤Ê¥¤¥Ù¥ó¥È¤ò¤³¤Ê¤·¤Æ¤¤¤Ê¤¤¾ì¹ç
		if( NPC_Util_GetStrFromStrWithDelim( szArg, "Pre_Not", szP, sizeof( szP ) ) == NULL ){
			print( "npc_bodylan:?¨Æ«e¨Æ¥óµ²§ôªº¤å¦r¨S¦³¿é¤J(%s)\n",	szArg );
			return;
		}
		sprintf( token,"%s", szP );
		buttontype=WINDOW_BUTTONTYPE_OK;	// OK
