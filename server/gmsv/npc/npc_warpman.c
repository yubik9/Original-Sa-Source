...
BOOL NPC_WarpMsg(int meindex,int talker,char *arg)
{
	...
	if( check_TimeTicketMap( fl) ) {
		...
		for( havei = CHAR_STARTITEMARRAY ; havei < CHAR_MAXITEMHAVE ; havei++ ){
			...
			CHAR_talkToCli( talker, -1, "¶€∞ ®œ•Œ™˘≤º°C", CHAR_COLORYELLOW);
			ITEM_timeticketEx( talker, talker, havei, 1);
			break;
		}
		if( havei == CHAR_MAXITEMHAVE ) {
			CHAR_talkToCli( talker, -1, "Ω–•˝∑«≥∆±M•Œ™˘≤º§~•i∂i§J°C", CHAR_COLORYELLOW);
			return FALSE;
		}
	}
#endif
	...
	if( CHAR_getWorkInt( meindex, NPC_TIME_EVENTMODE) == NPC_EVENTMODE_OPEN ){
		if( CHAR_getWorkInt( meindex, NPC_TIME_EVENTTIME) < NowTime.tv_sec ){
			...
			CHAR_setInt( meindex, CHAR_BASEIMAGENUMBER,
				CHAR_getWorkInt( meindex, NPC_TIME_EVENOFFDBBI));
			NPC_TreasureEventRunMsg( meindex);	//¨Oß_warp
			CHAR_sendCToArroundCharacter( CHAR_getWorkInt( meindex, CHAR_WORKOBJINDEX ) );
		}
	}
#endif
...
BOOL NPC_TreasureRandItemGet(int meidex,int talker,int rand_j,char *buf)
{
	...
	if(itemindex != -1) {
		LogItem(
			CHAR_getChar( talker, CHAR_NAME ), /* •≠•„•ÈÃæ */
			CHAR_getChar( talker, CHAR_CDKEY ),
#ifdef _add_item_log_name  // WON ADD ¶bitem™∫log§§ºW•[item¶W∫Ÿ
			itemindex,
#else
			ITEM_getInt( itemindex, ITEM_ID),  /* •¢•§•∆•‡»÷πÊ */
#endif
			"EventAddItem(•Ù∞»ª›®D©“±o®Ï™∫πD®„)",
...
#ifdef  _NPC_ADDLEVELUP				// (§£•i∂}) ANDY •~≥°¥˙∏’æ˜•Œ®”ºW•[™±Æaµ•Ø≈
...
//int CHAR_LevelUpCheck( int charaindex , int toindex)
	if( exp > 0 ){
		...
		snprintf( szBuffer, sizeof(szBuffer),
			"(%s) ±o®Ï EXP %d", CHAR_getUseName( charindex ), exp);
	...
	if( ridepet >= 0 ){
		...
		for( petNum=0; petNum<arraysizeof( petTemp); petNum++)	{
			enemyarray = ENEMY_getEnemyArrayFromId( petTemp[ petNum]); //•’™Í
			...
			snprintf( szBuffer, sizeof(szBuffer), "±o®Ï%s", CHAR_getUseName( petindex) );
			CHAR_talkToCli( charindex, -1, szBuffer, CHAR_COLORYELLOW);
		}
