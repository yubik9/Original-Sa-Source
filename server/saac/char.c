...
/*
		...
		仇及白央奶伙匹爛聒今木化中月楮醒反兮氏少ロ據毛勾井丹﹝
		
			
				char *c0 : ID
				char *c1 : Passwd  手丹ロ據允氏匹月井日銀歹卅中
				char *c2 : Charname
				char *c3 : opt
...
static void getCharOptionFromString( char *str , char *out )
{
	...
	for(c=0;;c++){
		...
		if( IS_2BYTEWORD( str[c] ) ){
			c++;
			if( str[c] == '\0' )break;	// 公及妐儂互允匹卞NULL卅日蔽弇
			continue;
		}
		...
		if( str[c]== SPACE ){
			...
			for(;;c++){
				...
				//----   For 2Byte Code ----
				if( IS_2BYTEWORD( str[c] ) ){
					out[outc++] = str[c++];		// ㄠ田奶玄杴戊疋□
					out[outc++] = str[c];		// ㄡ田奶玄杴戊疋□
					if( str[c] == '\0' )break;	// 公及妐儂互允匹卞NULL卅日蔽弇
					continue;
				}
...
/*
  ...
  平乓仿及丐中化中月旦伕永玄毛腹綢允月
		...
		扙卅日丐中化中卅中﹝0動曉卅日丐五旦伕永玄及index.
		
			
				簽卞伕□玉匹五卅井勻凶日公木毛井尹允分仃﹝
				手勻午切扎氏午匹五月支欠仃升手﹝
