...
/*
   ...
   char *pas : 天允歹□升
   char *charname : 平乓仿抩
   char *opt : 左皿扑亦件妐儂昫﹝ CharList及午五卞丟乒午仄化忒允樹扷
   char *charinfo : 白央奶伙卞忡繡允月平乓仿弁正及旦氾□正旦樹扷﹝
   int id : 丟永本□斥id
*/
#ifdef _NewSave
...
void saacproto_ACFMSetTAX_recv(int fd, int fm_tax, int index, int fmindex)
{
		...
		{			
			...
			saacproto_ACFMSetTAX_send(fd, FAILED, -1, 999, index);		// 不要怪我，不想再加欄位了，所以設成999
		}else{
			saacproto_ACFMSetTAX_send(fd, FAILED, -1, -1, index);		// -1 有許會有問題，要測試一下
		} 
